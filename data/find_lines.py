...
def find_lines(image):
    ...
    for cnt in contours:
        ...
        cv2.imshow("letter", threshed[y:(y + h), x:(x + w)])
        cv2.waitKey(0)
        rec_lett = pytesseract.image_to_string(threshed[y:(y + h), x:(x + w)], config='--psm 10',
                                               lang='pol')  # recognize the letter
        print(rec_lett)
    ...
    # plt.show()
    H, W = img.shape[:2]
    upuppers = [y for y in range(text_block_y, text_block_y + text_block_h, 1) if
                hist[y] <= th_lose and hist[y + 1] > th_lose]
    lolowers = [y for y in range(text_block_y, text_block_y + text_block_h, 1) if
                hist[y] > th_lose and hist[y + 1] <= th_lose]
    uppers = [y for y in range(text_block_y, text_block_y + text_block_h, 1) if
              hist[y] <= th_strict and hist[y + 1] > th_strict]
    lowers = [y for y in range(text_block_y, text_block_y + text_block_h, 1) if
              hist[y] > th_strict and hist[y + 1] <= th_strict]
...
def found_proper_line(uppers, lowers, word):
    i = 0
    print(len(uppers), i, uppers[i], (word[1] + word[3]), (uppers[i] > (word[1] + word[3])), lowers[i], word[1],
          (lowers[i] < word[1]))
