...
class EffectsView:
    ...
    def create_checkobxes(self):
        ...
        vbox.addWidget(chk)
        chk = QCheckBox()
        chk.stateChanged.connect(lambda: setattr(self.controller.original_effects,"minor_lines",not self.controller.original_effects.main_lines))
        chk.stateChanged.connect(lambda: self.controller.updated_drawing_effects(True))
        vbox.addWidget(chk)
        chk = QCheckBox()
        chk.stateChanged.connect(lambda: setattr(self.controller.original_effects,"text_block",not self.controller.original_effects.main_lines))
        chk.stateChanged.connect(lambda: self.controller.updated_drawing_effects(True))
        vbox.addWidget(chk)
        chk = QCheckBox()
        chk.stateChanged.connect(lambda: setattr(self.controller.original_effects,"words",not self.controller.original_effects.main_lines))
        chk.stateChanged.connect(lambda: self.controller.updated_drawing_effects(True))
        vbox.addWidget(chk)
        chk = QCheckBox()
        chk.stateChanged.connect(lambda: setattr(self.controller.original_effects,"letters",not self.controller.original_effects.main_lines))
        chk.stateChanged.connect(lambda: self.controller.updated_drawing_effects(True))
        vbox.addWidget(chk)
        hbox.addLayout(vbox)
