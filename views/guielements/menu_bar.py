from controllers.guielements.menu_bar_controller import MenuBarController
from PyQt5.QtWidgets import QAction,QMenu
class MenuBar():
    def __init__(self,parent)->None:
        self.controller = MenuBarController(parent)
        self.parent=parent
    
    # - File menu
    def get_file_menu(self):
        openAction = QAction("&Open files", self.parent ,shortcut="Ctrl+O")
        openAction.triggered.connect(lambda : self.controller.loadFiles())

        saveAction = QAction("&Save", self.parent ,shortcut="Ctrl+S")
        saveAction.triggered.connect(lambda: self.controller.saveImage)

        saveAllAction = QAction("&Save All",  self.parent)
        saveAllAction.triggered.connect(lambda: self.controller.saveAllImages)

        saveAsAction = QAction("&Save as",  self.parent ,triggered=self.controller.saveImageAs)
        saveAction.triggered.connect(lambda: self.controller.saveAllImages)

        saveMenu = QMenu("&Save",self.parent)
        saveMenu.addActions([saveAction, saveAsAction, saveAllAction])

        fileMenu = QMenu("&File",self.parent)
        fileMenu.addActions([openAction])
        fileMenu.addMenu(saveMenu)
        return fileMenu
    
    
